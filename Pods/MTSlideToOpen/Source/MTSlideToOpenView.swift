...
@objcMembers public class MTSlideToOpenView: UIView {
    ...
    public var labelText: String = "Swipe to Place Order" {
        didSet {
    ...
    @objc private func handlePanGesture(_ sender: UIPanGestureRecognizer) {
        ...
        case .ended:
            if translatedPoint >= xEndingPoint {
                // Change the thumbnail color and update the text
                thumnailImageView.backgroundColor = .white // Change thumbnail color to white
                textLabel.text = "Order Placed" // Update text to "Order Placed"
                textLabel.alpha = 0
                updateThumbnailXPosition(xEndingPoint)
